...
def lambda_handler(event, context):
    ...
    try:
        ...
        jwtHeaders = jwt.get_unverified_header(jwtToken)
        encryptedKey = base64.b64decode(jwtHeaders["KID"])
        response = kms_client.decrypt(CiphertextBlob=encryptedKey, KeyId=KMS_KEY_ID)
        decryptedKey = response["Plaintext"]
        decryptedEncodedKey = base64.b64encode(decryptedKey)
        payload = jwt.decode(jwtToken, decryptedEncodedKey, algorithms=["HS256"])
        principalId = payload["Username"]
        print(f"Validated {principalId} from jwt")
        isValid = True
